...
public class TextComparer {
    ...
    public static void main(String[] args) {
        if (args.length != 2) {
            System.err
                    .println("Usage: java TextComparer <file name> <file name>");
            System.exit(-1);
        }
        ...
        try {
            while ((line1 = reader1.readLine()) != null) {
                ...
                if (line2 == null) {
                    System.out.println("Second file is shorter (only " + count
                            + " lines)");
                    System.exit(-1);
                } else {
                    line2 = removeSpaces(line2);
                    if (!line1.equals(line2)) {
                        System.out.println("Comparison failure in line "
                                + count + ":");
            ...
            if (reader2.readLine() != null) {
                System.out.println("First file is shorter (only " + count
                        + " lines)");
                System.exit(-1);
            }
        } catch (IOException ioe) {
